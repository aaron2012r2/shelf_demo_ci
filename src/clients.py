...
def init_shelf_api_client():
    ...
    """
    token = st.secrets["keys_and_stuff"]["SHELF_API_TOKEN"]
    api_url = st.secrets["keys_and_stuff"]["SHELF_API_URL"]
...
def initialize_openai_client(llm_settings: dict) -> ChatOpenAI:
    ...
    """
    api_key = st.secrets["keys_and_stuff"]["OPENAI_API_KEY"]
...
def initialize_azure_openai_client(
    ...
    """
    api_key = st.secrets["keys_and_stuff"]["AZURE_OPENAI_API_KEY"]
    if api_key and api_key != "your_azure_openai_api_key_here":
        return AzureChatOpenAI(
            azure_deployment=st.secrets["keys_and_stuff"]["AZURE_DEPLOYMENT"],
            openai_api_version=st.secrets["keys_and_stuff"]["OPENAI_API_VERSION"],
            azure_endpoint=st.secrets["keys_and_stuff"]["AZURE_OPENAI_ENDPOINT"],
            openai_api_key=st.secrets["keys_and_stuff"]["AZURE_OPENAI_API_KEY"],
            **llm_settings,
        )
